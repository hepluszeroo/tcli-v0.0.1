...
export class AgentLoop {
   ...
   * 6. Generation counter increment to prevent ID collisions
   * 
  ...
  public terminate(): void {
    ...
    if (AgentLoop.DEBUG_TERMINATE_MODE) {
      ...
      const streamListeners = (this.currentStream as any)?.listenerCount?.('data') ?? 0;
      ...
      debugLog(
        ...
        `currentStream=${activeStreamRef} streamListeners=${streamListeners} ` +
        `callbacks={onItem:${onItemType}, onLoading:${onLoadingType}, ` +
    ...
    // and prevents ID collisions in future runs
    this.generation += 1;
    ...
    // Clean up all delivery timers explicitly
    for (const timer of this.deliveryTimers) {
      clearTimeout(timer);
    }
    
    // Explicitly remove all listeners from the current stream if present
    if (this.currentStream) {
      try {
        if (typeof (this.currentStream as any).removeAllListeners === 'function') {
          (this.currentStream as any).removeAllListeners();
        }
      } catch (e) {
        if (AgentLoop.DEBUG_TERMINATE_MODE) {
          debugLog(`[DEBUG_TERMINATE] Error removing stream listeners: ${e}`);
        }
      }
    }
    ...
    // We use a new Set rather than clear() to ensure the old object gets GC'd
    ...
    // Clear deliveryTimers with a fresh Set to ensure the old object is GC'd
    this.deliveryTimers = new Set();
    
    // Clear all object references that might retain memory
    // Using null rather than undefined to ensure complete reference breaking
    ...
    (this.instructions as any) = null;  // Instructions string can be large
    
    if (AgentLoop.DEBUG_TERMINATE_MODE) {
      // Report post-cleanup state for diagnostics
      debugLog(
      ...
      // Check for any remaining listeners in the process
      const processWarningListeners = process.listenerCount('warning');
      if (processWarningListeners > 5) { // 5 is a reasonable threshold for system listeners
        debugLog(`[DEBUG_TERMINATE] Warning: high warning listener count: ${processWarningListeners}`);
      }
    }
  ...
  constructor({
    ...
    // Create fresh abort controller with unique identity for each instance
    this.hardAbort = new AbortController();

    // Add abort listener with once:true to prevent listener accumulation
    ...
    if (AgentLoop.DEBUG_MODE) {
      ...
      // Register with instance tracker for leak detection
      if (typeof WeakRef === 'function' && AgentLoop.DEBUG_TERMINATE_MODE) {
        try {
          const { InstanceTracker } = require('../debug');
          InstanceTracker.get<AgentLoop>('AgentLoop').track(this);
          
          if (AgentLoop.activeCount % 10 === 0) {
            // Force GC and report counts periodically
            if (typeof global.gc === 'function') {
              global.gc();
              InstanceTracker.logCounts();
            }
          }
        } catch (e) {
          // Ignore errors in debug instrumentation
        }
      }
    }
  }
      ...
      while (turnInput.length > 0) {
        ...
        try {
          ...
          for await (const event of stream) {
            // Check if terminated during stream iteration to prevent continued processing
            if (this.terminated) {
              throw new Error('still streaming after terminate');
            }
      ...
      if (this.canceled || this.terminated) {
        if (this.currentStream) {
          try {
            ...
            // Abort the stream controller
            (this.currentStream as { controller?: { abort?: () => void } } | null)
              ?.controller?.abort?.();
              
            // Explicitly nullify the stream reference
            this.currentStream = null;
          } catch (e) {
            if (AgentLoop.DEBUG_TERMINATE_MODE) {
              debugLog(`[DEBUG_TERMINATE] Error cleaning up stream in finally: ${e}`);
            }
          }
        }
        ...
        if (this.pendingAborts.size > 0) {
          // Create a new Set rather than just clearing to help with GC
          this.pendingAborts = new Set();
        }
        ...
        if (AgentLoop.DEBUG_TERMINATE_MODE) {
          debugLog(
            `[DEBUG_TERMINATE] run() finally cleanup completed (id=${this.sessionId}): ` +
            `gen=${this.generation} deliveryTimers=${this.deliveryTimers.size} pendingAborts=${this.pendingAborts.size}`
          );
        }
      }
    }
