...
const MultilineTextEditorInner = (
  ...
  // Clean up buffer state on unmount to prevent stale state when the component remounts
  useEffect(() => {
    // Return cleanup function that runs when component unmounts
    return () => {
      if (buffer.current) {
        buffer.current.setText("");
      }
    };
  }, []);

  // -----------------------------------------------------------------------
  // Submit debounce – prevents duplicate onSubmit() calls when the terminal
  // emits both Control‑J and a plain carriage return in rapid succession.
  // A tiny 10 ms window is sufficient for the multiline Enter unit‑tests and
  // unnoticeable to human operators.
  const lastSubmitRef = useRef(0);
  const submitOnce = React.useCallback(
    (text: string) => {
      if (!onSubmit) return;
      const now = Date.now();
      if (now - lastSubmitRef.current < 10) {
    ...
    (input, key) => {
      ...
      if (input.startsWith("[") && input.endsWith("u")) {
        ...
        if (m && m[1] === "13") {
          ...
          if (hasCtrl) {
            if (onSubmit) {
              submitOnce(buffer.current.getText());
            }
          } else {
