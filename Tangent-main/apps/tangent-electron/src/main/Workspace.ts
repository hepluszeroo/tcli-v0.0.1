...
import CodexProcessManager from './codex_process_manager'
...
// Duplicate import removed above. Keep only single Logger import at the top.
...
export default class Workspace {
	...
	indexer: Indexer
	codexManager: CodexProcessManager | null = null
	...
	constructor(root: TreeNode) {
		...
		// -----------------------------
		// Feature-flag: Codex Integration
		// -----------------------------
		// Dynamically instantiate / dispose CodexProcessManager based on the
		// user setting so that the rest of Workspace logic remains untouched when
		// the flag is disabled (default).
			const codexLog = Logger.get('codex.flag')
			settings.enableCodexIntegration.subscribe(enabled => {
				if (process.env.INTEGRATION_TEST) {
					// eslint-disable-next-line no-console
					console.log('[debug] enableCodexIntegration changed →', enabled)
				}
				codexLog.info(`Codex integration ${enabled ? 'enabled' : 'disabled'}`)

				if (enabled) {
					// Instantiate a manager if we don’t yet have one **and** at least one
					// renderer window is attached (guards against the early-boot stage
					// where Workspace is created before any observers register).
					if (!this.codexManager) {
						if (process.env.INTEGRATION_TEST) console.log('[debug] creating CodexProcessManager')
						this.codexManager = new CodexProcessManager(this)
					}

					// Either newly-created or previously existing – in both cases ensure
					// the Codex child is actively running so the renderer can interact
					// straight away after the user flips the toggle.
					this.codexManager?.start()
				}
				else {
					// Flag turned off → stop the child and dispose manager instance.
					if (this.codexManager) {
						this.codexManager.cleanup()
	...
	addObserver(observer: WindowHandle) {
		...
		if (index < 0) {
			...
			// Initialize CodexProcessManager when first window is added **and**
			// feature flag is enabled.
			if (this.observers.length === 1 && !this.codexManager) {
				const currentSettings = getSettings()
				if (currentSettings.enableCodexIntegration.value) {
					this.codexManager = new CodexProcessManager(this)
					// Spawn the Codex child immediately so the renderer can use it
					// without requiring an explicit ipcRenderer.invoke('codex:start').
					this.codexManager.start()
				}
			}

			// If Codex manager already exists, expose it to the newly-opened window
			if (this.codexManager) {
				(observer.window.webContents as any).__codexManager = this.codexManager
				// Ensure the child is running in case the manager was created earlier
				// while no windows were attached (start() is idempotent).
				this.codexManager.start()
			}
		}
		return index
	...
	removeObserver(observer: WindowHandle) {
		...
		// Cleanup CodexProcessManager when all windows are closed
		if (this.observers.length === 0 && this.codexManager) {
			this.codexManager.cleanup();
			this.codexManager = null;
		}
	}
