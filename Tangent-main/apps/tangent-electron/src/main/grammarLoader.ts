...
async function loadVSCodeOnigurumaLib() {
	try {
		...
		// Typings of `loadWASM` expect an `ArrayBuffer | Response` but
		// `ArrayBufferLike` is produced by `.buffer` in Node ≥18.  A
		// straightforward cast keeps us compatible without bringing in the
		// DOM `Response` implementation during Electron main-process builds.
		await loadWASM(wasmBin as unknown as ArrayBuffer)
