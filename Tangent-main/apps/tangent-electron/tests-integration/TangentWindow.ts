...
import { WORKSPACE_VIEW_TIMEOUT } from './pathHelpers'
...
export default class TangentWindow {
	...
	async waitForReady() {
		// Wait until the initial HTML page has at least finished parsing; this is
		// extremely quick (<100 ms) but eliminates a race where the WorkspaceView
		// element is injected during the first micro-task after DOMContentLoaded.
		await this.page.waitForLoadState('domcontentloaded')
		// The first render of the Svelte workspace can take quite a few seconds
		// on cold Electron boots (especially in CI where GPU acceleration is
		// disabled).  Waiting for the more generic `networkidle` state proved
		// flaky, so we instead wait solely for the DOM element that indicates
		// the workspace view mounted.  The timeout is tuned dynamically via a
		// constant exported from pathHelpers so we can adapt CI vs. local runs
		// in one place.
		console.log('TangentWindow: Waiting for WorkspaceView element (state:attached)');
		await this.page.waitForSelector('main.WorkspaceView', {
		  state: 'attached',
		  timeout: WORKSPACE_VIEW_TIMEOUT
		});
		console.log('TangentWindow: WorkspaceView element found!');
	}
